...
def test_parser_accepts_polish_seconds_and_rejects_too_long():
    ...
    try:
        ...
        raise AssertionError("7-second direct render must be rejected")
...
def _write_request(tmp_path: Path, *, input_image: str | None = None) -> Path:
    job = tmp_path / "job"
    job.mkdir()
    request = {
        "schema": 2,
...
def test_worker_passes_duration_hq_and_input_image_to_ltx(monkeypatch, tmp_path):
