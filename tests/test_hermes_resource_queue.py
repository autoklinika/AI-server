...
spec=importlib.util.spec_from_file_location("hermes_resource_queue_test",ROOT/"tools/hermes_resource_queue.py")
...
def test_acquire_notifies_only_after_real_queue(monkeypatch):
    ...
    monkeypatch.setattr(mod,"_notify",lambda target,msg:notices.append((target,msg)))
    monkeypatch.setattr(mod,"_float_env",lambda name,default,lo,hi:0.0 if "NOTICE" in name else (0.001 if "POLL" in name else 30.0))
    monkeypatch.setattr(mod.ResourceLease,"start_heartbeat",lambda self:None)
    h=mod.acquire_resource(target="telegram:123",source="telegram-chat",queue_message="WAIT",start_message="START")
    assert notices==[("telegram:123","WAIT"),("telegram:123","START")]
    h.release()

def test_immediate_slot_sends_no_queue_noise(monkeypatch):
    monkeypatch.setattr(mod,"_json",lambda *a,**k:{"lease_id":"lease-B","job_id":8,"state":"active","released":True})
    monkeypatch.setattr(mod.ResourceLease,"start_heartbeat",lambda self:None)
    notices=[]
    monkeypatch.setattr(mod,"_notify",lambda target,msg:notices.append(msg))
    h=mod.acquire_resource(target="telegram:123",source="telegram-chat",queue_message="WAIT",start_message="START")
    assert notices==[]
    h.release()

def test_non_telegram_target_never_notifies(monkeypatch):
    monkeypatch.setattr(mod,"_json",lambda *a,**k:{"lease_id":"lease-C","job_id":9,"state":"active","released":True})
    monkeypatch.setattr(mod.ResourceLease,"start_heartbeat",lambda self:None)
    monkeypatch.setattr(mod,"_notify",lambda *a,**k:(_ for _ in ()).throw(AssertionError("notify")))
    h=mod.acquire_resource(target=None,source="ventilation")
    h.release()
