...
# Installed copy is rendered by install_privilege_bridge.sh. The repository
# copy intentionally remains non-operational until placeholders are replaced.
...
case "$stage" in
  ...
  L) WORKTREE="$ALLOWED_HOME/agent-worktrees/stage-l" ;;
  M) WORKTREE="$ALLOWED_HOME/agent-worktrees/stage-m" ;;
  O) WORKTREE="$ALLOWED_HOME/agent-worktrees/stage-o" ;;
  *) fail "stage_worktree" ;;
esac
case "$step" in
...
head_sha="$(ugit -C "$WORKTREE" rev-parse HEAD 2>/dev/null || true)"
[[ "$head_sha" == "$expected_sha" ]] || fail "head_sha"
branch="$(ugit -C "$WORKTREE" branch --show-current 2>/dev/null || true)"
[[ "$branch" == "$expected_branch" ]] || fail "branch"

[[ -z "$(ugit -C "$WORKTREE" status --porcelain --untracked-files=all)" ]] || fail "dirty"
...
ugit -C "$WORKTREE" ls-files --error-unmatch "$relative" >/dev/null 2>&1 \
  || fail "untracked_step"
...
committed_hash="$(ugit -C "$WORKTREE" show "$expected_sha:$relative" | sha256sum | awk '{print $1}')"
