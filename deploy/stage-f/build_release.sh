...
SOURCE_SHA="${STAGE_F_SOURCE_SHA:-$HEAD_SHA}"
[[ "$SOURCE_SHA" == "$HEAD_SHA" ]] || {
  echo "FAIL: Stage F source SHA does not match worktree HEAD" >&2
  exit 2
}
...
[[ "$RELEASE_ID" =~ ^stage-f-[a-f0-9]{12}$ ]] || fail "invalid Stage F release id"
[[ ! -e "$DEST" ]] || fail "destination already exists: $DEST"
ugit -C "$ROOT" cat-file -e "$SOURCE_SHA^{commit}" || fail "invalid source commit"

# Stage F intentionally permits Resource Manager / Gateway source changes.
...
# Both services come from the same exact committed source. Stage F permits
...
release:
  ...
  migration_version: hermes-supported-plugin-v1
...
python3 "$ROOT/deploy/stage-f/validate_release_metadata.py" "$DEST"
...
echo
say "Built Stage F release: $DEST"
cat "$DEST/RELEASE"
