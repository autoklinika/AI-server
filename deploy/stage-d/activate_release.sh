...
grep -qx 'stage=D' "$TARGET/RELEASE" || fail "target is not a Stage D release"
...
for unit in ai-bridge.service ai-gateway.service ai-bridge-analysis.service; do
  ...
  if systemctl cat "$unit" | grep -q '95-ai-platform-release.conf'; then
...
curl -fsS $GATEWAY_HEALTH_URL >/dev/null || fail "AI Gateway health failed"
curl -fsS $BRIDGE_HEALTH_URL >/dev/null || fail "AI Bridge health failed"
...
echo "===== STAGE D CUTOVER PASS ====="
say "previous=$PREVIOUS"
say "current=$(readlink -f "$CURRENT")"
say "rollback: $ROOT/deploy/stage-d/rollback_release.sh"
