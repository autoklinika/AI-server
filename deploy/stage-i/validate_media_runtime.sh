...
EXPECTED_STAGE="${1:?usage: $0 <H|I>}"
[[ "$EXPECTED_STAGE" == "H" || "$EXPECTED_STAGE" == "I" ]] || {
  echo "FAIL: expected stage must be H or I" >&2
  exit 2
}

CURRENT="/opt/ai-platform/current"
WRAPPER="/usr/local/bin/generate-video-ltx23"
OUTPUT_DIR="$(mktemp -d /tmp/stage-i-media-smoke.XXXXXX)"
...
try:
    ...
    matching = [
        ...
        if item.get("source") == "stage-i-media-smoke" and item.get("priority") == 50
    ]
...
assert_idle()
print("PASS: Stage I real media smoke")
PY
...
echo "PASS: Stage I media runtime validation"
