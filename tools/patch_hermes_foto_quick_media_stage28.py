...
def _return_match(text: str):
    ...
    match = matches[0]
    window = text[max(0, match.start() - 3200):match.start()]
    ...
    if STAGE26_ARGS not in window or STAGE26_ROUTE not in window:
        raise PatchError("Stage26 args/route bridge must already be installed before Stage28")
    return match


def _block(indent: str) -> str:
    return (
        f'{indent}# {MARKER}: pass the exact current-message image path to deterministic /foto.\n'
        f'{indent}# Quick-command dispatch returns immediately, so no normal vision enrichment follows.\n'
        f'{indent}if command == "foto":\n'
        f'{indent}    from gateway.run import _event_media_is_image as _stage28_media_is_image\n'
        f'{indent}    _stage28_image_paths = [\n'
        f'{indent}        str(_path)\n'
        f'{indent}        for _idx, _path in enumerate(getattr(event, "media_urls", None) or [])\n'
        f'{indent}        if _stage28_media_is_image(event, _idx)\n'
        f'{indent}    ]\n'
        f'{indent}    if _stage28_image_paths:\n'
        f'{indent}        exec_cmd = (\n'
        f'{indent}            "HERMES_FOTO_INPUT_IMAGE="\n'
        f'{indent}            + _stage26_shlex.quote(_stage28_image_paths[0])\n'
        f'{indent}            + " " + exec_cmd\n'
        f'{indent}        )\n'
    )
...
def patch_text(text: str) -> str:
    ...
    if count > 1:
        raise PatchError("duplicate Stage28 foto media marker")
    if count == 1:
    ...
    if patched.count(MARKER) != 1:
        raise PatchError("internal error: Stage28 marker not unique")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Patch modern Hermes quick /foto with exact current-turn image path")
