...
with urllib.request.urlopen("http://127.0.0.1:11435/health", timeout=1) as r:
    data=json.load(r)
assert data.get("status") == "ok" and data.get("ollama") == "ok", data
PY
        then
            return 0
        fi
...
say "Temporary test only: no permanent parallelism setting will remain."

# Refuse to start while any gateway work is active/queued.
"$PYTHON" - <<'PY'
import json, urllib.request
...
run_variant() {
    ...
    wait_ollama || { say "FAIL: Ollama did not recover for np=$parallel"; exit 1; }

    # Warm the model so model-load latency does not contaminate the pair benchmark.
    "$PYTHON" - "$MODEL" <<'PY'
...
PY
    ...
    # Read the complete command line from /proc and verify the requested runner parallelism.
    "$PYTHON" - "$parallel" <<'PY'
from pathlib import Path
import sys
want=f"-np {sys.argv[1]}"
commands=[]
for p in Path('/proc').iterdir():
    if not p.name.isdigit():
        continue
    try:
        ...
        continue
    if 'llama-server' in cmd:
        commands.append((p.name,cmd))
for pid,cmd in commands:
    print(f"runner pid={pid}: {cmd}")
if not any(want in cmd for _,cmd in commands):
    raise SystemExit(f"FAIL: no llama-server runner reports {want}")
print(f"PASS: runner confirmed {want}")
PY
...
prompts=[
    "Twórz kolejne krótkie techniczne zdania po polsku o lokalnym serwerze AI. Nie kończ przed limitem odpowiedzi. Oznaczenie A.",
    "Twórz kolejne krótkie techniczne zdania po polsku o lokalnym serwerze AI. Nie kończ przed limitem odpowiedzi. Oznaczenie B.",
]
...
if any(r is None or not r.get("done") for r in results):
    raise SystemExit(f"FAIL: incomplete benchmark results: {results}")
...
summary={
    ...
    "requests":results,
    "min_mem_available_kb":min((s["mem_available_kb"] for s in samples if s["mem_available_kb"] is not None),default=None),
    "max_llama_runner_rss_kb":max((s["runner_rss_kb"] for s in samples),default=0),
...
print(f"MemAvailable delta np2-vs-np1: {summary['np2_vs_np1_min_mem_delta_gib']:.2f} GiB")
print(f"runner RSS delta np2-vs-np1:   {summary['np2_vs_np1_runner_rss_delta_gib']:.2f} GiB")
...
sudo systemctl daemon-reload
sudo systemctl restart ollama.service
wait_ollama || { say "FAIL: Ollama did not recover after restoring original config"; exit 1; }
sudo systemctl start ai-gateway.service
wait_gateway || { say "FAIL: AI Gateway did not recover after Ollama restore"; exit 1; }

# Warm the normal production model again.
