...
MARKER = "AI_SERVER_DISCORD_QUEUE_VOICE_V1"
TARGET = "gateway/run_turn_runner.py"

OLD = '''    def voice_ack_callback(self, call_id, tool_name, args):
...
def patch_text(text: str) -> str:
    if text.count(MARKER) > 1:
        raise PatchError("duplicate Discord queue voice marker")
    if MARKER in text:
    ...
    if patched.count(MARKER) != 1:
        raise PatchError("Discord queue voice marker insertion failed")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print("patched Discord queue voice v1")
        return 0
