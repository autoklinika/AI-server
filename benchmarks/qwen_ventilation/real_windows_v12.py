...
"""Read-only archived-real-window validation for v12 structured decisions.

The script reads the same six archived windows used for v11.x validation. Qwen
returns only a structured decision. Python validates selected fact IDs and renders
all operator-facing prose and numbers deterministically. No rows are written to
...
def main() -> int:
    ...
    try:
        for label, start_text, end_text in WINDOWS:
                ...
                record.update(
                    {
                        "skipped": True,
                        "reason": (
                            f"sample_count={len(rows)} below gate {settings.analysis_min_samples}"
                        ),
                    }
                )
            ...
            decision_packet = build_decision_packet_from_compact(compact)

            chat = client.chat_structured(
                model=settings.ollama_model,
                messages=build_decision_prompt_from_compact(compact),
                response_schema=response_schema,
                think=ANALYSIS_THINK,
                temperature=settings.analysis_temperature,
            )
            decision = VentilationDecisionV12.model_validate_json(chat.content)
            validate_decision(compact, decision)
            result = render_result(compact, decision)

            duration = None if chat.total_duration_ns is None else round(chat.total_duration_ns / 1e9, 3)
            print(
                f"status={decision.status} reasons={decision.reason_codes} "
                f"rec={decision.recommendation_code}"
            )
            print(f"facts={decision.selected_fact_ids}")
    ...
    output = RESULTS_DIR / f"real_windows_v12_{stamp}.json"
